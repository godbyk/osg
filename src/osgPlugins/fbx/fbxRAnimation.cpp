    ...
    const char* targetName, const char* channelName)
{
    ...
    return readFbxChannels(
        fbxProp.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X),
        fbxProp.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_Y),
        fbxProp.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_Z),
        fbxProp.Get(),
...
std::string OsgFbxReader::readFbxAnimation(FbxNode* pNode, const char* targetName)
{
    ...
    {
        FbxAnimStack* pAnimStack = FbxCast<FbxAnimStack>(fbxScene.GetSrcObject(FBX_TYPE(FbxAnimStack), i));
        ...
        {
            FbxAnimLayer* pAnimLayer = pAnimStack->GetMember(FBX_TYPE(FbxAnimLayer), j);
